...
from views.new_transmision_seccionador import Ui_NewBook
from db.books import buscar_usuario_acceso,insert_book_transmision_seccionador,insert_book_transmision_seccionador_ubicacion_plano,insert_book_transmision_seccionador_ubicacion_esquema
...
#
    ...
    def open_new_window_esquemas(self):
        from controllers.new_window_subestacion_seccionador_esquema import NewBookWindow
        window= NewBookWindow(self)
        window.show()
    def open_new_window_plano(self):
        from controllers.new_window_subestacion_seccionador_plano import NewBookWindow
        window= NewBookWindow(self)
        window.show()
    ...
    def add_book(self):
        ...
        if self.check_inputs():
            ...
            self.label_nota_obligatoria.hide()
            data = (self.Usuario, inputNombre, inputCodigo, inputCelda, cbCalificacion, cbTipo,cbTipoInstalacion, inputTension, inputMarca, inputAnio, cbEstado, inputFecha)

            #self.AnadirNuevos(dni,visitante,entidadempresa,motivovisita,aquienvisita,autoriza,areavisitada) 
            if insert_book_transmision_seccionador(data):
                #self.clean_inputs()
                ...
                insert_book_transmision_seccionador_ubicacion_esquema(data2)
                insert_book_transmision_seccionador_ubicacion_plano(data3)
    ...
    def populate_inputs(self, seccionador_id):
        ...
        if conn:
            try:
                ...
                # Consulta a la tabla Seccionador
                sql = "SELECT * FROM Seccionador WHERE COD_SECCIONADOR = ?"
                ...
                if row:
                    ...
                    inputMarca = str(row[8])
                    inputAnio = str(row[9])
                    cbEstado = str(row[10])
                    inputFecha = str(row[11])

                    # Poblar los campos de entrada de la tabla Seccionador
                    ...
                    # Consulta a la tabla Seccionador_Ubicacion_Esquema
                    sql_esquema = "SELECT * FROM Seccionador_Ubicacion_Esquema WHERE COD_SECCIONADOR = ?"
                    ...
                    if row_esquema:
                        ...
                        # Poblar los campos de entrada de la tabla Seccionador_Ubicacion_Esquema
                        self.inputX_2.setText(inputX_2)
                        self.inputY_2.setText(inputY_2)
                        self.inputANG_2.setText(inputANG_2)

                    # Consulta a la tabla Seccionador_Ubicacion_Plano_Planta
                    sql_plano = "SELECT * FROM Seccionador_Ubicacion_Plano_Planta WHERE COD_SECCIONADOR = ?"
                    ...
                    if row_plano:
                        ...
                        # Poblar los campos de entrada de la tabla Seccionador_Ubicacion_Plano_Planta
                ...
                print("Error al obtener detalles del seccionador:", str(e))
            finally:
                if conn:
                    conn.close()
