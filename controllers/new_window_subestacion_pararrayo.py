...
from views.new_transmision_pararrayo import Ui_NewBook
from db.books import buscar_usuario_acceso,insert_book_transmision_pararrayo,insert_book_transmision_pararrayo_ubicacion_esquema,insert_book_transmision_pararrayo_ubicacion_plano
...
#
    ...
    def open_new_window_coordenadas(self):
        from controllers.new_window_subestacion_pararrayo_esquema import NewBookWindow
        window= NewBookWindow(self)
        window.show()
    def open_new_window_plano(self):
        from controllers.new_window_subestacion_pararrayo_plano import NewBookWindow
        window= NewBookWindow(self)
        window.show()
    ...
    def add_book(self):
        ...
        if self.check_inputs():
            ...
            self.label_nota_obligatoria.hide()
            data = (self.Usuario, inputNombre, inputCodigo, cbCalificacion, inputTension, inputMarca, inputAnio, cbEstado, inputFecha)

            #self.AnadirNuevos(dni,visitante,entidadempresa,motivovisita,aquienvisita,autoriza,areavisitada) 
            if insert_book_transmision_pararrayo(data):
                #self.clean_inputs()
                ...
                insert_book_transmision_pararrayo_ubicacion_esquema(data2)
                insert_book_transmision_pararrayo_ubicacion_plano(data3)
    ...
    def populate_inputs(self, book_id):
        ...
        if conn:
            try:
                ...
                # Consulta a la tabla Pararrayo
                sql = "SELECT * FROM Pararrayo WHERE COD_PARARRAYO = ?"
                ...
                if row:
                    ...
                    inputMarca = str(row[5])
                    inputAnio = str(row[6])
                    cbEstado = str(row[7])
                    inputFecha = str(row[8])

                    # Poblar los campos de entrada de la tabla Pararrayo
                    ...
                    # Consulta a la tabla Pararrayo_Ubicacion_Esquema
                    sql_esquema = "SELECT * FROM Pararrayo_Ubicacion_Esquema WHERE COD_PARARRAYO = ?"
                    ...
                    if row_esquema:
                        ...
                        # Poblar los campos de entrada de la tabla Pararrayo_Ubicacion_Esquema
                        self.inputX_2.setText(inputX_2)
                        self.inputY_2.setText(inputY_2)
                        self.inputANG_2.setText(inputANG_2)

                    # Consulta a la tabla Pararrayo_Ubicacion_Plano_Planta
                    sql_plano = "SELECT * FROM Pararrayo_Ubicacion_Plano_Planta WHERE COD_PARARRAYO = ?"
                    ...
                    if row_plano:
                        ...
                        # Poblar los campos de entrada de la tabla Pararrayo_Ubicacion_Plano_Planta
