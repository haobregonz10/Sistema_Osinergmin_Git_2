...
from views.edit_book_window_locadores import EditBookFormLocadores
from db.books import EncontrarAutorizaArea_locadores, EncontrarDni_locadores, EncontrarNombreVisitante_locadores, ExisteAutoriza_locadores, ExisteDni_locadores, ExisteEntidad_locadores, ExisteMotivo_locadores, ExisteNombreVisitante_locadores, ExisteVisita_locadores, UpdateAutoriza_locadores, UpdateVisitante_locadores, insert_NuevoAutoriza_locadores, insert_NuevoDNI_locadores, insert_NuevoEntidad_locadores, insert_NuevoMotivo_locadores, insert_NuevoVisita_locadores, select_all_EntidadTabla_locadores, select_all_motivoTabla_locadores, select_all_personalFeban_locadores, select_all_visitantesDNI_locadores, select_all_visitantesNombres_locadores, select_book_by_id,UpdateAutoriza,EncontrarAutorizaArea, select_book_by_id_locadores, update_book,select_all_personalFeban,select_all_areaTabla, select_all_EntidadTabla,select_all_motivoTabla,select_all_visitantesDNI, select_all_visitantesNombres,EncontrarDni,ExisteDni,ExisteAutoriza,ExisteEntidad,ExisteMotivo,ExisteVisita,insert_NuevoEntidad,insert_NuevoAutoriza,insert_NuevoDNI,insert_NuevoMotivo,insert_NuevoVisita,EncontrarNombreVisitante,ExisteNombreVisitante,UpdateVisitante, update_book_locadores, select_all_visitantesNombres_locadores,EncontrarEntidadVisitante_locadores,EncontrarMotivoVisitante_locadores,EncontrarVisitaVisitante_locadores,EncontrarAutorizaVisitante_locadores,EncontrarAreaVisitante_locadores,EncontrarPisoVisitante_locadores,update_NuevoDNI_locadoresV2,EncontrarAreasDelBn,EncontrarPisoSeleccionado,EncontrarAreaSeleccionada
from pys2_msgboxes import msg_boxes
from PySide2.QtGui import QFont
import re
import os
import shutil
class EditBookWindow(QWidget, EditBookFormLocadores):
    ...
    def __init__(self, parent=None, _id=None):
        ...
        listaPersonalFeban = select_all_personalFeban_locadores()
        completer = QCompleter(listaPersonalFeban, self)
        completer.setFilterMode(Qt.MatchContains)
        completer.popup().setFont(fontMain)

        listaPersonalFeban2 = select_all_personalFeban_locadores()
        completer2 = QCompleter(listaPersonalFeban2, self)
        completer2.setFilterMode(Qt.MatchContains)
        completer2.popup().setFont(fontMain)

        listaVisitantesDNIS = select_all_visitantesDNI_locadores()
        listaVisitantesNombres = select_all_visitantesNombres_locadores()
        listaEntidades = select_all_EntidadTabla_locadores()
        listaMotivos = select_all_motivoTabla_locadores()
    ...
    def populate_inputs(self):
        ...
        data = select_book_by_id_locadores(self._id)
    ...
    def edit_book(self):
        ...
        if self.check_inputs():
            ...
            if update_book_locadores(self._id, data):
                msg_boxes.correct_msgbox("¡Modificación Exitosa!","¡Se ha modificado el registro exitosamente!")
                self.parent.refresh_table_from_child_window_locadores()
                print("MODIF EXIT")
                self.close()
            '''
            if(len(dni)!=8):
    ...
    def VerificarDni(self,dni):
        ...
        if (ExisteDni_locadores(dni)):
            print("DNI SI EXISTE - Verificacion!")
            nombreEncontrado= EncontrarDni_locadores(dni)
    ...
    def VerificarAutoriza(self,nombreautoriza):
        ...
        if (ExisteAutoriza_locadores(nombreautoriza)):
            print("DNI SI EXISTE - Verificacion!")
            areaEncontrada= EncontrarAutorizaArea_locadores(nombreautoriza)
    ...
    def VerificarNombreVisitante(self,NombreVisitante):
        ...
        if (ExisteNombreVisitante_locadores(NombreVisitante)):
            print("NombreVisitante SI EXISTE - Verificacion!")


            DniEncontrado= EncontrarNombreVisitante_locadores(NombreVisitante)
            EntidadEncontrado= EncontrarEntidadVisitante_locadores(NombreVisitante)
            MotivoEncontrado= EncontrarMotivoVisitante_locadores(NombreVisitante)
            VisitaEncontrado= EncontrarVisitaVisitante_locadores(NombreVisitante)
            AutorizaEncontrado= EncontrarAutorizaVisitante_locadores(NombreVisitante)
            AreaEncontrado= EncontrarAreaVisitante_locadores(NombreVisitante)
            PisoEncontrado= EncontrarPisoVisitante_locadores(NombreVisitante)
    ...
    def AnadirNuevos(self, dni,visitante,entidadempresa,motivovisita,aquienvisita,autoriza,area,piso):
        if (ExisteDni_locadores(dni)):
            print("DNI SI EXISTE!")
            update_NuevoDNI_locadoresV2(dni,visitante,entidadempresa,motivovisita,aquienvisita,autoriza,area,piso)
        else:
            print("DNI NO EXISTE!")
            insert_NuevoDNI_locadores(dni,visitante,entidadempresa,motivovisita,aquienvisita,autoriza,area,piso)
            

        if (ExisteEntidad_locadores(entidadempresa)):
            print("Entidad SI EXISTE!")
        else:
            print("Entidad NO EXISTE!")
            insert_NuevoEntidad_locadores(entidadempresa) 
        
        if (ExisteMotivo_locadores(motivovisita)):
            print("Motivo SI EXISTE!")
        else:
            print("Motivo NO EXISTE!")
            insert_NuevoMotivo_locadores(motivovisita) 
        
        if (ExisteVisita_locadores(aquienvisita)):
            print("Visita SI EXISTE!")
        else:
            print("Visita NO EXISTE!")
            insert_NuevoVisita_locadores(aquienvisita) 

        if (ExisteAutoriza_locadores(autoriza)):
            print("Autoriza SI EXISTE!")
            UpdateAutoriza_locadores(autoriza,area)
        else:
            print("Autoriza NO EXISTE!")
            insert_NuevoAutoriza_locadores(autoriza,area)    
    ...
    def ActualizarVisitante(self,dni,visitante):
        ...
        if nombreEncontrado==visitante:
            ...
            UpdateVisitante_locadores(dni,visitante)
            print("Actualizado!!!")
    def ListaAreas(self):
