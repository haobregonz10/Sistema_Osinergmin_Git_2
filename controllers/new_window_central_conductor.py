...
from views.new_central_conductor import Ui_NewBook
from db.books import delete_esquema_conductor_central,delete_plano_conductor_central,buscar_usuario_acceso,insert_book_conductor_central_generacion,select_plano_conductor_central,select_esquema_conductor_central
...
class NewBookWindow(QDialog,Ui_NewBook):
    def __init__(self, parent=None,_codCentral=None,_codigo=None):
        ...
        #self.btnAddEsquema.clicked.connect(self.open_new_window_esquema)
        #self.btnAddPlanta.clicked.connect(self.open_new_window_planta)
...
#
        ...
        self.populate_table_plano(select_plano_conductor_central(self._codCentral,self.inputCodigo.text()))
        self.populate_table_esquema(select_esquema_conductor_central(self._codCentral,self.inputCodigo.text()))
        #self.cancelButton.clicked.connect(self.close)
        ...
        if len(selected_row)!=0:
            ...
            if respuesta== QMessageBox.Yes:
                if selected_row:
                    ...
                    if delete_plano_conductor_central(self.Usuario,self._codCentral,self.inputCodigo.text(),book_id):
        ...
        if len(selected_row)!=0:
            ...
            if respuesta== QMessageBox.Yes:
                if selected_row:
                    ...
                    if delete_esquema_conductor_central(self.Usuario,self._codCentral,self.inputCodigo.text(),book_id):
    ...
    def refresh_table_from_child_window(self):
        ...
        data = select_plano_conductor_central(self._codCentral,self.inputCodigo.text())
        data2 = select_esquema_conductor_central(self._codCentral,self.inputCodigo.text())
    ...
    def open_new_window_coordenadas(self,codCentral):
        if(self.inputCodigo.isEnabled()):
            ...
            from controllers.new_window_central_conductor_esquema import NewBookWindow
    ...
    def open_new_window_plano(self,codCentral):
        if(self.inputCodigo.isEnabled()):
            ...
            from controllers.new_window_central_conductor_plano import NewBookWindow
            codCelda= self.inputCodigo.text()
            codCentral= self.inputCentral.text()
            window= NewBookWindow(self,codCelda,codCentral)
            window.exec_()

    def open_new_window_esquema(self):
        from controllers.new_window_central_conductor_esquema import NewBookWindow
        window= NewBookWindow(self)
        window.show()
    def open_new_window_planta(self):
        from controllers.new_window_central_conductor_plano import NewBookWindow
        window= NewBookWindow(self)
        window.show()
...
#
        ...
        #inputZ=inputZ.strip()
#
        #inputSecuencia_2 = self.inputANG_2.text()
...
#
        ...
        if self.check_inputs():
            ...
            self.label_nota_obligatoria.hide()
            data = (self.Usuario,inputCentral,inputCodigo,inputTension,cbEstado,inputFecha) 
            
            if insert_book_conductor_central_generacion(data):
                #self.clean_inputs()
                #data2 = (self.Usuario, inputCentral, inputCodigo, inputSecuencia_2, inputX_2, inputY_2)
                #data3 = (self.Usuario, inputCentral,inputCodigo,cbFase, inputSecuencia, inputX, inputY, inputZ)
                
                #insert_book_conductor_central_generacion_ubicacion_esquema(data2)
                #insert_book_conductor_central_generacion_planta(data3)
...
#
    ...
    def populate_inputs(self, book_id):
        ...
        if conn:
            try:
                cur = conn.cursor()
                # Consulta a la tabla Conductor_Central_Generacion
                sql = f"SELECT * FROM Conductor_Central_Generacion WHERE COD_CONDUCTOR = ?"
                ...
                if row:
                    ...
                    inputTension = str(row[3])
                    cbEstado = str(row[4])
                    inputFecha = str(row[5])

                    # Poblar los campos de entrada de la tabla Conductor_Central_Generacion
                    ...
                    ## Consulta a la tabla Conductor_Central_Generacion_Ubicacion_Esquema
                    #sql_esquema = f"SELECT * FROM Conductor_Central_Generacion_Ubicacion_Esquema WHERE COD_CONDUCTOR = ?"
                    ...
                    #    # Poblar los campos de entrada de la tabla Conductor_Central_Generacion_Ubicacion_Esquema
                    #    self.inputANG_2.setText(inputSecuencia_2)
                    #    self.inputX_2.setText(inputX_2)
                    #    self.inputY_2.setText(inputY_2)
#
                    ## Consulta a la tabla Conductor_Central_Generacion_Ubicacion_Plano_Planta
                    #sql_planta = f"SELECT * FROM Conductor_Central_Generacion_Ubicacion_Plano_Planta WHERE COD_CONDUCTOR = ?"
                    #cur.execute(sql_planta, (book_id,))
                    #row_planta = cur.fetchone()
                    #if row_planta:
                    #    # Obtener los valores de la fila
                    #    cbFase = str(row_planta[3])
                    #    inputSecuencia = str(row_planta[4])
                    #    inputX = str(row_planta[5])
                    #    inputY = str(row_planta[6])
                    #    inputZ = str(row_planta[7])
                    #    # Poblar los campos de entrada de la tabla Conductor_Central_Generacion_Ubicacion_Plano_Planta
