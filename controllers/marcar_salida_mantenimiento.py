...
class MarcarSalida(QWidget, MarcarSalidaFormMantenimiento):
    def __init__(self, parent=None, _id=None):
        ...
        listaPersonalFeban = select_all_personalFeban_mantenimiento()
        completer = QCompleter(listaPersonalFeban, self)
        completer.setFilterMode(Qt.MatchContains)
        completer.popup().setFont(fontMain)

        listaPersonalFeban2 = select_all_personalFeban_mantenimiento()
        completer2 = QCompleter(listaPersonalFeban2, self)
        completer2.setFilterMode(Qt.MatchContains)
        completer2.popup().setFont(fontMain)

        listaVisitantesDNIS = select_all_visitantesDNI_mantenimiento()
        listaVisitantesNombres = select_all_visitantesNombres_mantenimiento()
        listaEntidades = select_all_EntidadTabla_mantenimiento()
        listaMotivos = select_all_motivoTabla_mantenimiento()
    ...
    def populate_inputs(self):
        ...
        data = select_book_by_id_mantenimiento(self._id)
    ...
    def edit_book(self):
        ...
        if self.check_inputs():
            ...
            if update_book_mantenimiento(self._id, data):
                msg_boxes.marc_salida_msgbox("Marcar Salida","¡Se marcó la salida correctamente!")
                self.parent.refresh_table_from_child_window_mantenimiento()
                print("MODIF EXIT")
                self.close()
    ...
    def VerificarDni(self,dni):
        ...
        if (ExisteDni_mantenimiento(dni)):
            print("DNI SI EXISTE - Verificacion!")
            nombreEncontrado= EncontrarDni_mantenimiento(dni)
    ...
    def VerificarNombreVisitante(self,NombreVisitante):
        ...
        if (ExisteNombreVisitante_mantenimiento(NombreVisitante)):
            DniEncontrado= EncontrarNombreVisitante_mantenimiento(NombreVisitante)
            EntidadEncontrado= EncontrarEntidadVisitante_mantenimiento(NombreVisitante)
            MotivoEncontrado= EncontrarMotivoVisitante_mantenimiento(NombreVisitante)
            VisitaEncontrado= EncontrarVisitaVisitante_mantenimiento(NombreVisitante)
            AutorizaEncontrado= EncontrarAutorizaVisitante_mantenimiento(NombreVisitante)
            AreaEncontrado= EncontrarAreaVisitante_mantenimiento(NombreVisitante)
            PisoEncontrado= EncontrarPisoVisitante_mantenimiento(NombreVisitante)
    ...
    def AnadirNuevos(self, dni,visitante,entidadempresa,motivovisita,aquienvisita,autoriza,area,piso):
        if (ExisteDni_mantenimiento(dni)):
            ...
            print("DNI NO EXISTE!")
            insert_NuevoDNI_mantenimiento(dni,visitante,entidadempresa,motivovisita,aquienvisita,autoriza,area,piso)
        
        if (ExisteEntidad_mantenimiento(entidadempresa)):
            print("Entidad SI EXISTE!")
        else:
            print("Entidad NO EXISTE!")
            insert_NuevoEntidad_mantenimiento(entidadempresa) 
        
        if (ExisteMotivo_mantenimiento(motivovisita)):
            print("Motivo SI EXISTE!")
        else:
            print("Motivo NO EXISTE!")
            insert_NuevoMotivo_mantenimiento(motivovisita) 
        
        if (ExisteVisita_mantenimiento(aquienvisita)):
            print("Visita SI EXISTE!")
        else:
            print("Visita NO EXISTE!")
            insert_NuevoVisita_mantenimiento(aquienvisita) 

        if (ExisteAutoriza_mantenimiento(autoriza)):
            print("Autoriza SI EXISTE!")
            UpdateAutoriza_mantenimiento(autoriza,area)
        else:
            print("Autoriza NO EXISTE!")
            insert_NuevoAutoriza_mantenimiento(autoriza,area)    
    ...
    def ActualizarVisitante(self,dni,visitante):
        ...
        if nombreEncontrado==visitante:
            ...
            UpdateVisitante_mantenimiento(dni,visitante)
            print("Actualizado!!!")

    def VerificarAutoriza(self,nombreautoriza):
        print("EL nombreautoriza es:"+nombreautoriza)
        if (ExisteAutoriza_mantenimiento(nombreautoriza)):
            print("DNI SI EXISTE - Verificacion!")
            areaEncontrada= EncontrarAutorizaArea_mantenimiento(nombreautoriza)
