...
from views.new_central_pararrayo import Ui_NewBook
from db.books import buscar_usuario_acceso,insert_book_pararrayo_central_generacion,insert_book_pararrayo_central_generacion_ubicacion_esquema,insert_book_pararrayo_central_generacion_planta
...
#
    ...
    def open_new_window_esquemas(self):
        from controllers.new_window_central_pararrayo_esquema import NewBookWindow
        window= NewBookWindow(self)
        window.show()
    def open_new_window_plano(self):
        from controllers.new_window_central_pararrayo_plano import NewBookWindow
        window= NewBookWindow(self)
        window.show()
    ...
    def add_book(self):
        ...
        if self.check_inputs():
            ...
            self.label_nota_obligatoria.hide()
            data = (self.Usuario,inputCentral,inputCodigo,inputTension,inputMarca,inputAnio,cbEstado,inputFecha) 
            
            if insert_book_pararrayo_central_generacion(data):
                #self.clean_inputs()
                ...
                insert_book_pararrayo_central_generacion_ubicacion_esquema(data2)
                insert_book_pararrayo_central_generacion_planta(data3)
...
#
    ...
    def populate_inputs(self, book_id):
        ...
        if conn:
            try:
                cur = conn.cursor()
                # Consulta a la tabla Pararrayo_Central_Generacion
                sql = f"SELECT * FROM Pararrayo_Central_Generacion WHERE COD_PARARRAYO = ?"
                ...
                if row:
                    ...
                    inputMarca = str(row[4])
                    inputAnio = str(row[5])
                    cbEstado = str(row[6])
                    inputFecha = str(row[7])

                    # Poblar los campos de entrada de la tabla Pararrayo_Central_Generacion
                    ...
                    # Consulta a la tabla Pararrayo_Central_Generacion_Ubicacion_Esquema
                    sql_esquema = f"SELECT * FROM Pararrayo_Central_Generacion_Ubicacion_Esquema WHERE COD_PARARRAYO = ?"
                    ...
                    if row_esquema:
                        ...
                        # Poblar los campos de entrada de la tabla Pararrayo_Central_Generacion_Ubicacion_Esquema
                        self.inputX_2.setText(inputX_2)
                        self.inputY_2.setText(inputY_2)
                        self.inputANG_2.setText(inputANG_2)

                    # Consulta a la tabla Pararrayo_Central_Generacion_Ubicacion_Plano_Planta
                    sql_planta = f"SELECT * FROM Pararrayo_Central_Generacion_Ubicacion_Plano_Planta WHERE COD_PARARRAYO = ?"
                    ...
                    if row_planta:
                        ...
                        # Poblar los campos de entrada de la tabla Pararrayo_Central_Generacion_Ubicacion_Plano_Planta
