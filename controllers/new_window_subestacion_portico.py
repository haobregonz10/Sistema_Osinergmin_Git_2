from PySide2.QtWidgets import QMessageBox,QDialog,QTableWidgetItem, QWidget, QFileDialog,QCompleter,QAbstractItemView,QHeaderView
from PySide2.QtCore import Qt, QDate
from PySide2.QtGui import *
import sqlite3
from sqlite3 import Error
from views.new_transmision_portico import Ui_NewBook
from db.books import buscar_usuario_acceso,delete_book_Portico_Estructura,insert_book_portico,insert_book_portico_travesanio,select_cod_estructura_portico_sub
...
class NewBookWindow(QDialog,Ui_NewBook):
    def __init__(self, parent=None,_codCentral=None,_codigo=None):
        ...
        self.populate_table_estructura(select_cod_estructura_portico_sub(_codCentral,_codigo))
    ...
    def open_new_window_estructura(self):
        if(self.inputCodigo.isEnabled()):
            print("DEBE AGREGAR")
            msg_boxes.alert_msgbox_2("¡Error!","¡Primero debe agregar el código del Pórtico!")
        else:        
            from controllers.new_window_subestacion_portico_estructura import NewBookWindow
    ...
    def open_new_window_travesanio(self):
        from controllers.new_window_subestacion_portico_travesanio import NewBookWindow
    ...
    def add_book(self):
        ...
        if self.check_inputs():
            ...
            self.label_nota_obligatoria.hide()
            data = (self.Usuario,inputCentral,inputCodigo,cbCalificacion,cbTipo,inputAltura,cbEstado,inputFecha) 

            if insert_book_portico(data):
                data2 = (self.Usuario,inputCentral,inputCodigo,inputAltura_2,inputEstructura,inputEstructura2) 
                insert_book_portico_travesanio(data2)
                #self.clean_inputs()
...
#
    ...
    def populate_inputs(self, book_id):
        ...
        if conn:
            try:
                cur = conn.cursor()
                # Consulta a la tabla Portico
                sql = "SELECT * FROM Portico WHERE COD_PORTICO = ?"
                ...
                if row:
                    ...
                    cbTipo = str(row[4])
                    inputAltura = str(row[5])
                    cbEstado = str(row[6])
                    inputFecha = str(row[7])

                    # Poblar los campos de entrada de la tabla Portico
                    ...
                    # Consulta a la tabla Portico_Travesanio
                    sql_travesanio = "SELECT * FROM Portico_Travesanio WHERE COD_PORTICO = ?"
                    ...
                    if row_travesanio:
                        ...
                        # Poblar los campos de entrada de la tabla Portico_Travesanio
    ...
    def populate_combobox(self):
        ...
        # Llenar el combo de Código del tipo de Pórtico
        cb_options_tipo_portico = ("", "Acero", "Madera","Concreto")
    ...
    def refresh_table_from_child_window(self):
        ...
        print("REFRESH1")
        data = select_cod_estructura_portico_sub(self.inputCentral.text(),self.inputCodigo.text())
        print("REFRESH2")
        ...
        if len(selected_row)!=0:
            ...
            if respuesta== QMessageBox.Yes:
                if selected_row:
                    ...
                    if delete_book_Portico_Estructura(self.inputCodigo.text(),book_id):
